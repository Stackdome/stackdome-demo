...
5. Run mir-reflect /tmp/mirror. You can see images: after the first render, and whenever the score stalls, open /tmp/mirror/measure/original.png and rebuild.png with the read tool and compare them by eye (at most 4 image reads in a run; they are costly). Its SIZE, BOX and TEXT lines say exactly where the rebuild differs and by how much; act on them directly and do not write your own image-analysis scripts. Fix the largest problem first (size, then layout, then type, then colour) and run it again. Keep improving pass after pass until the score is 95 or more, or the CLOCK line says 20 minutes; most of the score usually comes in passes three to five.
